...
struct Book
{
	...
	void toString() {
		printf("Book: %d, %s, %s, %s, %d, %s, %.2f\n", id,
			name, author, publishingHouse, pageNum, genre, price);
	}
};
...
int main() {
	Book books[] = {
	 { 1,"Metro2033", "Dmitro glyknovsky",
	 "Rainbow", 350,"sci-fi",125.50 },
		{2, "The Da Vinch Code", "Den Brow",
		"Sunshine",450,"Detective",200},
		{3, "Sherlok Holmes", "Arthur Conan Doyle",
	"Twillight", 189, "Detective", 350.50},
	{4, "Harry Potter and Death Hallows", "J.K.Rowling",
	"Moonlight", 500, "Fantasy", 415.75}
	};
	printf("Books befere sorting:\n");
	printBooks(books, 4);
...
bool  sortBooks(Book* books, int size, const char* option)
{
	...
	if (strcmp(option, "name") == 0) {
		...
		for (int i = 0; i < size - 1; i++) {
		for (int j = 0; j < size -1; j++) {
			if (books[j], books[j + 1].pageNum > books[j +
				1].pageNum) {
				std::swap(books[j], books[j + 1]);
		      }
			}
		...
		for (int i = 0; i < size - 1; i++) {
			for (int j = 0; j < size - 1; j++) {
				if (books[j], books[j + 1].price > books[j +
					1].price) {
					std::swap(books[j], books[j + 1]);
				   }
				 }
